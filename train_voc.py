...
for epoch in range(EPOCHS):
    ...
    for epoch_step, (batch_imgs, batch_boxes, batch_classes, loc_target, cls_target) in enumerate(train_loader):
        ...
        print("global_steps:%d epoch:%d steps:%d/%d cls_loss:%.4f reg_loss:%.4f loc_anchor:%.4f cls_anchor:%.4f lr=%.4e"%\
            (GLOBAL_STEPS,epoch+1,epoch_step+1,steps_per_epoch, loss_fcos[0], loss_fcos[1], loss_anchor[0], loss_anchor[1], lr))
        '''
        print(
    ...
    torch.save(model.state_dict(), "/mnt/hdd1/benkebishe01/paper_voc/diou/voc_epoch%d_loss%.4f.pth" % (epoch+1, loss_avg.item()))
    # torch.save(model.state_dict(), "/mnt/hdd1/benkebishe01/FCOS/fsaf/voc_epoch%d_loss%.4f.pth" % (epoch + 1, loss_avg.item()))
